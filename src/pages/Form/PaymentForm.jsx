...
const PaymentForm = () => {
  ...
  const onChangeInvoice = (id, e) => {
    ...
    if (target === "invoice_selected") {
      ...
      if (value === false) {
        newPaymentData["total_invoice_amount"] -=
          (changingObj.paying_amount !== ""
            ? parseFloat(changingObj.paying_amount)
            : 0) +
          (changingObj.adjustment_amount !== ""
            ? parseFloat(changingObj.adjustment_amount)
            : 0);
        changingObj["paying_amount"] = "";
        changingObj["adjustment_amount"] = "";
      } else {
      ...
      if (
        parseFloat(value) +
          (changingObj.adjustment_amount !== ""
            ? parseFloat(changingObj.adjustment_amount)
            : 0) >
        parseFloat(changingObj.total_amount)
      ) {
        value =
          changingObj.paying_amount !== ""
            ? parseFloat(changingObj.paying_amount)
        ...
        const preChangingObjValue =
          changingObj.paying_amount !== ""
            ? parseFloat(changingObj.paying_amount)
            : 0;
        const netValue =
          (value !== "" ? parseFloat(value) : 0) - preChangingObjValue;
  ...
  function loadDueInvoices(paymentData) {
    setPaymentData({ ...paymentData, total_invoice_amount: 0 });
  ...
  const onSubmit = (e) => {
    ...
    const finalSelectedInvoices = selectedInvoices.map((inv) => {
      const payingAmount =
        inv.paying_amount !== "" &&
        inv.paying_amount !== 0 &&
        inv.adjustment_amount !== null
          ? parseFloat(inv.paying_amount)
          : 0;
      const adjAmount =
        inv.adjustment_amount !== "" &&
        inv.adjustment_amount !== 0 &&
        inv.adjustment_amount !== null
          ? parseFloat(inv.adjustment_amount)
          : 0;
      ...
      const newInv = {
        ...
        invoice_status:
          parseFloat(payingAmount + adjAmount) < parseFloat(inv.total_amount)
            ? "partial_paid"
            : "paid",
      };
            ...
            >
              ...
              <Card.Body>
                ...
                <Table striped bordered hover>
                  ...
                  <thead>
                    {Object.keys(invoicesData).length > 0 &&
                      Object.entries(invoicesData).map((inv) => {
                        return (
                          <tr key={"inv" + inv[0]}>
                            <td>
                              <Form.Check
                            ...
                            <td>
                              <Form.Group as={Col}>
                                <Form.Control
                                  type="number"
                                  max={
                                    inv[1].total_amount - inv[1].paying_amount
                                  }
                                  name="adjustment_amount"
                                  readOnly={
                                    !inv[1].invoice_selected ||
                                    inv[1].paying_amount >= inv[1].total_amount
                                  }
                    ...
                    {Object.keys(invoicesData).length < 1 ? (
                      <b> No due invoice found </b>
                    ) : null}
                  </thead>
                </Table>
                <Form.Group as={Col}>
